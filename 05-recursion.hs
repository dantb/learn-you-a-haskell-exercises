...
fib x
  | x == 0 = [0]
  | x == 1 = [1, 0]
  | otherwise = let l@(a:b:as) = fib (x - 1) in (a+b):l
...
stepReverseSign n step = (abs n + step) * if n <= 0 then 1 else -1
...
piCalc' :: (Ord a, Fractional a, Integral b) => a -> a -> a -> b -> (a, b)
piCalc' prevDenom prevPi tol count =
  let
    newPi    = prevPi + (4 / prevDenom)
    newDenom = stepReverseSign prevDenom 2
  in
    if abs (newPi - prevPi) <= tol then (newPi, count)
    else piCalc' newDenom newPi tol (count + 1)
